...
namespace fnecore.EDAC
{
    ...
    /// 
    /// </summary>
    public class Golay24128
    {
        private static readonly uint[] ENCODING_TABLE_23127 = {
        ...
        private static readonly uint[] ENCODING_TABLE_24128 = {
        ...
        private static readonly uint[] DECODING_TABLE_23127 = {
        ...
        private const uint X22 = 0x00400000;   /* vector representation of X^{22} */
        private const uint X11 = 0x00000800;   /* vector representation of X^{11} */
        private const uint MASK12 = 0xfffff800;/* auxiliary vector for testing */
        private const uint GENPOL = 0x00000c75;/* generator polinomial, g(x) */
        ...
        {
            uint syndrome = getSyndrome23127(code);
            uint error_pattern = DECODING_TABLE_23127[syndrome];
        ...
        /// <returns></returns>
        public static bool decode24128(uint code, ref uint _out)
        {
            uint syndrome = getSyndrome23127(code >> 1);
            uint error_pattern = DECODING_TABLE_23127[syndrome] << 1;

            _out = code ^ error_pattern;

            bool valid = (FneUtils.CountBits(syndrome) < 3U) || ((FneUtils.CountBits(_out) & 1) == 0);
            _out >>= 12;
        ...
        /// <param name="_out"></param>
        /// <returns></returns>
        public static bool decode24128(byte[] bytes, ref uint _out)
        {
            if (bytes == null)
                throw new NullReferenceException("bytes");

            uint code = (uint)((bytes[0U] << 16) | (bytes[1U] << 8) | (bytes[2U] << 0));
            return decode24128(code, ref _out);
        }
        ...
        /// <returns></returns>
        public static void decode24128(ref byte[] data, byte[] raw, uint msglen)
        {
            uint i = 0;                  // decoded byte counter
            uint j = 0;                  // encoded byte counter
            uint r0, r1, r2, r3, r4, r5; // six 8-bit bytes
            uint v0, v1;                 // two 24-bit encoded symbols
            uint m0_hat = 0, m1_hat = 0; // two 12-bit decoded symbols
            ...
            for (i = 0; i < msglen - r; i += 3) {
                ...
                decode24128(v0, ref m0_hat);
                decode24128(v1, ref m1_hat);

                // unpack two 12-bit symbols into three 8-bit bytes
                data[i + 0] = (byte)(((m0_hat >> 4) & 0xff));
                data[i + 1] = (byte)(((m0_hat << 4) & 0xf0) | ((m1_hat >> 8) & 0x0f));
                data[i + 2] = (byte)(((m1_hat) & 0xff));
        ...
        /// <returns></returns>
        public static uint encode23127(uint data)
        {
        ...
        /// <param name="data">Data to encode with Golay FEC.</param>
        /// <returns></returns>
        public static uint encode24128(uint data)
        {
        ...
        public static void encode24128(ref byte[] data, byte[] raw, uint msglen)
        {
            uint j = 0;
            uint s0, s1, s2;            // three 8-bit symbols
            uint m0, m1;                // two 12-bit symbols (uncoded)
            uint v0, v1;                // two 24-bit symbols (encoded)

            // determine remainder of input length / 3
            uint r = msglen % 3;

            for (uint i = 0; i < msglen - r; i += 3)
            {
                ...
                data[j + 0] = (byte)((v0 >> 16) & 0xff);
                data[j + 1] = (byte)((v0 >> 8) & 0xff);
                data[j + 2] = (byte)((v0) & 0xff);
                data[j + 3] = (byte)((v1 >> 16) & 0xff);
                data[j + 4] = (byte)((v1 >> 8) & 0xff);
                data[j + 5] = (byte)((v1) & 0xff);

                j += 6;
            }

            // if input length isn't divisible by 3, encode last 1 or two bytes
            for (uint i = msglen - r; i < msglen; i++)
            {
        ...
        /// <returns></returns>
        private static uint getSyndrome23127(uint pattern)
        {
            uint aux = X22;
            if (pattern >= X11)
            {
                while ((pattern & MASK12) > 0)
                {
                    while ((aux & pattern) == 0)
                        aux = aux >> 1;
